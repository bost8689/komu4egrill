...
class ControllerExtensionShippingXshippingpro extends Controller {
	...
	public function exportMethod($tab_id) {
		...
		$heading = array('От','До','Стоимость','Диапазон кол-ва','Неполный диапазон');
		...
		$language_id = $this->config->get('config_language_id');
		$method_name = (!isset($method_data['name'][$language_id]) || !$method_data['name'][$language_id]) ? 'Способ без названия '.$no_of_tab : $method_data['name'][$language_id]; 
	...
	private function getFormData($data, $new_tab = false)
	{
		...
		foreach($data['method_data'] as $single_method) {
			...
			foreach ($data['languages'] as $language) { 
				...
				if(!isset($method_data['name'][$language['language_id']]) || !$method_data['name'][$language['language_id']])$method_data['name'][$language['language_id']]='Способ '.$no_of_tab; 
				...
				if(!isset($method_data['mask'][$language['language_id']]) || !$method_data['mask'][$language['language_id']])$method_data['mask'][$language['language_id']]='';
				...
				.'<label class="col-sm-2 control-label" for="input-mask'.$no_of_tab.''.$language['language_id'].'">'.$data['text_mask_price'].'</label>'
				.'<div class="col-sm-10"><input id="input-mask'.$no_of_tab.''.$language['language_id'].'" class="form-control" type="text" name="xshippingpro[mask]['.$language['language_id'].']" value="'.$method_data['mask'][$language['language_id']].'" /></div>'
				.'</div>'
			...
			for($sg=1; $sg<=$data['shipping_xshippingpro_sub_groups_count'];$sg++) { 
				$return.='<option '.(($method_data['group']==$sg)?'selected':'').' value="'.$sg.'">Группа '.$sg.'</option>';
			} 
			$return.='</select></div>'
			.'</div>'
			.'</div>'
	...
	private function getMethodList($data) {
		...
		foreach($data['method_data'] as $single_method) {
			...
			{
				$return[$no_of_tab] = isset($method_data['name'][$this->config->get('config_language_id')])? $method_data['name'][$this->config->get('config_language_id')] : 'Способ '.$no_of_tab;
			}
			else {
	...
	private function getSubGroups($data) {
		...
		for($i=1; $i<=$data['shipping_xshippingpro_sub_groups_count']; $i++) {
			...
			$return .='<tr>
				<td class="text-left">Группа'.$i.'</td>
				<td class="text-left">
