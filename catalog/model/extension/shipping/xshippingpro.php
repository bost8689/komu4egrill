...
class ModelExtensionShippingXshippingpro extends Model {
	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['xshippingpro'.$no_of_tab] = array(
				...
				'text'         => ($cost==0 && $xshippingpro['mask'][$language_id])? $xshippingpro['mask'][$language_id].$quote_desc: $this->currency->format($this->tax->calculate($cost, $xshippingpro['tax_class_id'], $this->config->get('config_tax')),$currency_code).$quote_desc
				);
		 }
	}
	...
	$method_data = array(
		'code'       => 'xshippingpro',
		'title'      => ($xshippingpro_heading) ? html_entity_decode($xshippingpro_heading) : '',
		'quote'      => $quote_data,
	...
	if ($xshippingpro_debug && $debugging  && !$is_admin && !$is_quote) { 
		...
		<i style="color:red;">У вас включена отладка в модуле Мои способы доставки</i><br />';
		foreach($debugging as $debug){
			echo '<b>'.$debug['name'].' ('.$debug['index'].')</b> разрешено для <u>'.implode(',',$debug['filter']).'</u><br />';
		}
		echo '</div>';
	}
